inherited pageCliente: TpageCliente
  Align = alClient
  Caption = 'pageCliente'
  PixelsPerInch = 96
  TextHeight = 17
  inherited cardPanelCrud: TCardPanel
    inherited cardPesquisa: TCard
      inherited pnlTituloPesquisa: TPanel
        inherited lblTituloPesquisa: TLabel
          Width = 178
          Caption = 'Cadastro de Clientes'
          ExplicitWidth = 178
        end
      end
    end
    inherited cardEdicao: TCard
      inherited pnlAbas: TPanel
        inherited btnAbaDadoGeral: TSpeedButton
          ExplicitLeft = -6
          ExplicitTop = 6
        end
        object SpeedButton3: TSpeedButton [1]
          Left = 288
          Top = 0
          Width = 144
          Height = 63
          Align = alLeft
          Caption = 'Observa'#231#245'es'
          ImageIndex = 10
          ImageName = 'login16'
          Images = VirtualImageListCrud
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clGrayText
          Font.Height = -11
          Font.Name = 'Segoe UI'
          Font.Style = []
          Layout = blGlyphTop
          ParentFont = False
          OnClick = btnAbaDadoGeralClick
          ExplicitLeft = 8
          ExplicitHeight = 64
        end
        object SpeedButton4: TSpeedButton [2]
          Left = 144
          Top = 0
          Width = 144
          Height = 63
          Align = alLeft
          Caption = 'Endere'#231'o e Contato'
          ImageIndex = 10
          ImageName = 'login16'
          Images = VirtualImageListCrud
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clGrayText
          Font.Height = -11
          Font.Name = 'Segoe UI'
          Font.Style = []
          Layout = blGlyphTop
          ParentFont = False
          OnClick = btnAbaDadoGeralClick
          ExplicitLeft = 8
          ExplicitHeight = 64
        end
        inherited Panel1: TPanel
          ExplicitTop = 63
        end
      end
    end
  end
  inherited VirtualImageListCrud: TVirtualImageList
    Images = <
      item
        CollectionIndex = 0
        CollectionName = 'cancelarEsair'
        Disabled = False
        Name = 'cancelarEsair'
      end
      item
        CollectionIndex = 1
        CollectionName = 'cancelarEsair16'
        Disabled = False
        Name = 'cancelarEsair16'
      end
      item
        CollectionIndex = 2
        CollectionName = 'editar'
        Disabled = False
        Name = 'editar'
      end
      item
        CollectionIndex = 3
        CollectionName = 'editar16'
        Disabled = False
        Name = 'editar16'
      end
      item
        CollectionIndex = 4
        CollectionName = 'excluir'
        Disabled = False
        Name = 'excluir'
      end
      item
        CollectionIndex = 5
        CollectionName = 'excluir16'
        Disabled = False
        Name = 'excluir16'
      end
      item
        CollectionIndex = 6
        CollectionName = 'novo'
        Disabled = False
        Name = 'novo'
      end
      item
        CollectionIndex = 7
        CollectionName = 'novo16'
        Disabled = False
        Name = 'novo16'
      end
      item
        CollectionIndex = 8
        CollectionName = 'salvar'
        Disabled = False
        Name = 'salvar'
      end
      item
        CollectionIndex = 9
        CollectionName = 'salvar16'
        Disabled = False
        Name = 'salvar16'
      end
      item
        CollectionIndex = 10
        CollectionName = 'login16'
        Disabled = False
        Name = 'login16'
      end>
  end
  inherited ImageCollectionCrud: TImageCollection
    Images = <
      item
        Name = 'cancelarEsair'
        SourceImages = <
          item
            Image.Data = {
              
              454E44AE426082}
          end>
      end
      item
        Name = 'cancelarEsair16'
        SourceImages = <
          item
            Image.Data = {
              }
          end>
      end
      item
        Name = 'editar'
        SourceImages = <
          item
            Image.Data = {
              
              426082}
          end>
      end
      item
        Name = 'editar16'
        SourceImages = <
          item
            Image.Data = {
              
              454E44AE426082}
          end>
      end
      item
        Name = 'excluir'
        SourceImages = <
          item
            Image.Data = {
              }
          end>
      end
      item
        Name = 'excluir16'
        SourceImages = <
          item
            Image.Data = {
              }
          end>
      end
      item
        Name = 'novo'
        SourceImages = <
          item
            Image.Data = {
              
              0ECAD00000000049454E44AE426082}
          end>
      end
      item
        Name = 'novo16'
        SourceImages = <
          item
            Image.Data = {
              
              AE426082}
          end>
      end
      item
        Name = 'salvar'
        SourceImages = <
          item
            Image.Data = {
              
              0049454E44AE426082}
          end>
      end
      item
        Name = 'salvar16'
        SourceImages = <
          item
            Image.Data = {
              
              51F29142120000000049454E44AE426082}
          end>
      end
      item
        Name = 'login16'
        SourceImages = <
          item
            Image.Data = {
              
              426082}
          end>
      end>
  end
end
